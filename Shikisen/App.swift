...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuDelegate {
    ...
    static let defaultViewSize = NSSize(width: 1000, height: 800)
    var window: NSWindow!
    var view: SubMTKView!
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        ...
        let endClosure: () -> () = { [weak self] in
            ...
            if urls.count == 1
                && urls[0].pathExtension == Document.FileType.shikisendata.filenameExtension {
...
final class SubMTKView: MTKView, MTKViewDelegate,
        ...
        Task { @MainActor in
            ...
            case .ok:
                let loadResult = await URL.load(prompt: "Replace".localized,
                                                fileTypes: [Document.FileType.shikisendata,
                                                            Document.FileType.rasendata])
                switch loadResult {
    ...
    @objc func exportDatabase(_ sender: Any) {
        Task { @MainActor in
            ...
            let result = await URL.export(name: "User", fileType: Document.FileType.shikisendata,
