...
extension Bezier {
    ...
    func t(withLength length: Double) -> Double? {
        ...
        guard length < maxLength else { return length == maxLength ? 1 : nil }
        ...
        return bisection(minT: 0, maxT: 1)
    }
    func t(withLength length: Double, flatness: Int) -> Double {
        var d = 0.0, oldP = p0
        let nd = 1 / Double(flatness)
        for i in 1 ... flatness {
            let t = Double(i) * nd
    ...
    func intersects(_ otherEdge: Edge) -> Bool {
        !intersections(otherEdge).isEmpty
    }
    ...
    func intersections(_ edge: Edge) -> [BezierIntersection] {
        ...
        func intersection(with t: Double) -> BezierIntersection? {
            ...
            let p = position(withT: t)
            guard let et = edge.t(from: p) else { return nil }
        ...
        if d > 0 {
            ...
            if let bi0 = intersection(with: t0) {
                if let bi1 = intersection(with: t1) {
                    return [bi0, bi1]
                } else {
                    return [bi0]
                }
            } else if let bi = intersection(with: t1) {
                return [bi]
            }
        } else if d == 0 {
            let t = -0.5 * db / da
    ...
    func intersections(_ other: Bezier) -> [BezierIntersection] {
        ...
        if isLineaer {
            if other.isLineaer {
                ...
                let results = other.intersections(Edge(p0, p1))
                return results.map { BezierIntersection(t: $0.otherT, otherT: $0.t,
                                                        otherDirection: $0.otherDirection.reversed,
                                                        point: $0.point) }
            }
        } else {
            if other.isLineaer {
                return intersections(Edge(other.p0, other.p1))
            } else {
    ...
    func t(atX x: Double) -> [Double] {
        guard !isLineaer else {
            if let t = Edge(p0, p1).t(atX: x) {
                return [t]
            } else {
                return []
            }
        }
        let a = p1.x + p0.x - 2 * cp.x
        let b = -2 * p0.x + 2 * cp.x
        let c = p0.x - x
        let d = b * b - 4 * a * c
        guard d > 0 else {
