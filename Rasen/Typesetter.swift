...
extension Typesetter {
    ...
    func paddingSize(from typeline: Typeline) -> Size {
        paddingSize(fromSpacing: typeline.spacing)
    }
    func halfPaddingSize(from typeline: Typeline) -> Size {
        halfPaddingSize(fromSpacing: typeline.spacing)
    }
    func spacingSize(from typeline: Typeline) -> Size {
        spacingSize(fromSpacing: typeline.spacing)
    }
    
    var spacingTypoBoundsEnabledEmpty: Rect {
        spacingTypoBounds ?? Rect().outset(by: Typesetter.spacing(from: typobute))
    }
    
    func paddingSize(fromSpacing spacing: Double) -> Size {
        switch typobute.orientation {
        case .horizontal:
            .init(width: typobute.font.size,
                  height: spacing / 2)
        case .vertical:
            .init(width: spacing / 2,
                  height: typobute.font.size)
        }
    }
    func halfPaddingSize(fromSpacing spacing: Double) -> Size {
        switch typobute.orientation {
        case .horizontal:
            .init(width: typobute.font.size / 2,
                  height: spacing / 2)
        case .vertical:
            .init(width: spacing / 2,
    ...
    func spacingSize(fromSpacing spacing: Double) -> Size {
        switch typobute.orientation {
        case .horizontal:
            .init(width: 0, height: spacing / 2)
        case .vertical:
            .init(width: spacing / 2, height: 0)
        }
    }
}
extension Typesetter {
...
extension Typesetter {
    ...
    func containsFirst(_ p: Point, offset: Double) -> Bool {
        ...
        return switch typobute.orientation {
        case .horizontal:
            typeline.frame.outset(by: offset, .top).contains(p)
        case .vertical:
            typeline.frame.outset(by: offset, .right).contains(p)
        }
    }
    func containsLast(_ p: Point, offset: Double) -> Bool {
        ...
        return switch typobute.orientation {
        case .horizontal:
            typeline.frame.outset(by: offset, .bottom).contains(p)
        case .vertical:
            typeline.frame.outset(by: offset, .left).contains(p)
        }
    }
    ...
    func firstEdge(from typeline: Typeline, offset: Double) -> Edge {
        switch typobute.orientation {
        case .horizontal: typeline.frame.outset(by: offset, .top).topEdge
        case .vertical: typeline.frame.outset(by: offset, .right).rightEdge
        }
    }
    ...
    func lastEdge(from typeline: Typeline, offset: Double) -> Edge {
        switch typobute.orientation {
        case .horizontal: typeline.frame.outset(by: offset, .bottom).bottomEdge
        case .vertical: typeline.frame.outset(by: offset, .left).leftEdge
        }
    }
    var width: Double {
        switch typobute.orientation {
        case .horizontal: typoBounds?.width ?? 0
        case .vertical: typoBounds?.height ?? 0
        }
    }
    var height: Double {
        switch typobute.orientation {
        case .horizontal: typoBounds?.height ?? 0
        case .vertical: typoBounds?.width ?? 0
        }
    }
    ...
    var firstBounds: Rect? {
        if let typeline = typelines.first {
            return typeline.firstTypoBounds() + typeline.origin
        } else{
            return firstReturnBounds
        }
    }
    var lastBounds: Rect? {
        if let typeline = typelines.last, !typeline.isLastReturnEnd {
    ...
    var firstReturnBounds: Rect? {
        ...
        case .horizontal:
            if let typeline = typelines.first {
                ...
                let font = typobute.font
                let d = typobute.font.size + Typesetter.spacing(from: typobute)
                let p = Point(0, d)
                return Rect(x: p.x, y: p.y - font.size / 2,
                            width: 0, height: font.size)
            }
    ...
    func characterIndex(for point: Point,
        ...
        guard let typeline = self.typeline(for: point) else {
            ...
            case .horizontal:
                ...
                } else if typelines.isEmpty {
                    let font = typobute.font
                    let spacing = Typesetter.spacing(from: typobute)
                    let frame = Rect(x: 0, y: -font.size / 2,
                                     width: 0, height: font.size)
                        .outset(by: isHalfPadding ?
                                halfPaddingSize(fromSpacing: spacing) :
                                    paddingSize(fromSpacing: spacing))
                    if frame.contains(point) {
            ...
            case .vertical:
                if let typeline = typelines.last, typeline.isLastReturnEnd {
                    ...
                    let font = typobute.font
                    let spacing = Typesetter.spacing(from: typobute)
                    let frame = Rect(x: -font.size / 2, y: 0,
                                 width: font.size, height: 0)
                        .outset(by: isHalfPadding ?
                                halfPaddingSize(fromSpacing: spacing) :
                                    paddingSize(fromSpacing: spacing))
    ...
    func cursorBounds(at i: String.Index,
        ...
        return switch typobute.orientation {
        case .horizontal:
            .init(x: p.x - d, y: p.y - font.size / 2 * hr - d,
                  width: d * 2, height: font.size * hr + d * 2)
        case .vertical:
            .init(x: p.x - font.size / 2 * hr - d, y: p.y - d,
    ...
    var maxTypelineWidthPath: Path {
        ...
        return switch typobute.orientation {
        case .horizontal:
            .init(Edge(Point(w + b.minX, b.minY),
                       Point(w + b.minX, b.maxY)))
        case .vertical:
            .init(Edge(Point(b.minX, b.maxY - w),
