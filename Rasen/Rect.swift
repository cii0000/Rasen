...
extension RectCorner: Protobuf {
    ...
    var pb: PBRectCorner {
        switch self {
        case .minXMinY: .minXminY
        case .minXMaxY: .minXmaxY
        case .maxXMinY: .maxXminY
        case .maxXMaxY: .maxXmaxY
        }
    }
}
...
extension Rect {
    ...
    func inset(by d: Double, _ lrtb: LRTB) -> Rect {
        switch lrtb {
        case .bottom:
            .init(x: origin.x, y: origin.y + d,
                  width: size.width, height: size.height - d)
        case .left:
            .init(x: origin.x + d, y: origin.y,
    ...
    func intersection(_ other: Rect) -> Rect? {
        ...
        return w >= 0 && h >= 0 ?
            Rect(x: minX, y: minY, width: w, height: h) : nil
    }
    ...
    func intersectionEdges(_ other: Rect,
                           minLength: Double = 10) -> [Edge] {
        var edges = [Edge]()
        if maxX < other.maxX {
            if let nyRange = yRange.intersection(other.yRange,
                                                 minLength: minLength) {
                edges.append(.init(.init(maxX, nyRange.lowerBound),
                                   .init(maxX, nyRange.upperBound)))
            }
        }
        if minX > other.minX {
            if let nyRange = yRange.intersection(other.yRange,
                                                 minLength: minLength) {
                edges.append(.init(.init(minX, nyRange.lowerBound),
                                   .init(minX, nyRange.upperBound)))
            }
        }
        if maxY < other.maxY {
            if let nxRange = xRange.intersection(other.xRange,
                                                 minLength: minLength) {
                edges.append(.init(.init(nxRange.lowerBound, maxY),
                                   .init(nxRange.upperBound, maxY)))
            }
        }
        if minY > other.minY {
            if let nxRange = xRange.intersection(other.xRange,
                                                 minLength: minLength) {
                edges.append(.init(.init(nxRange.lowerBound, minY),
                                   .init(nxRange.upperBound, minY)))
            }
        }
        return edges
    }
    ...
    func minLine(_ other: Rect) -> Pathline? {
        ...
        if other.maxX < minX {
            if other.maxY < minY {
                return Pathline([minXMinYPoint, other.maxXMaxYPoint])
            } else if other.minY <= maxY {
                let y = yRange.intersection(other.yRange)?.mid ?? midY
                return Pathline([Point(minX, y), Point(other.maxX, y)])
            } else {
                return Pathline([minXMaxYPoint, other.maxXMinYPoint])
            }
        } else if other.minX <= maxX {
            let x = xRange.intersection(other.xRange)?.mid ?? midX
            if other.maxY < minY {
                return Pathline([Point(x, minY), Point(x, other.maxY)])
            } else {
                return Pathline([Point(x, maxY), Point(x, other.minY)])
            }
        } else {
            if other.maxY < minY {
                return Pathline([maxXMinYPoint, other.minXMaxYPoint])
            } else if other.minY <= maxY {
                let y = yRange.intersection(other.yRange)?.mid ?? midY
                return Pathline([Point(maxX, y), Point(other.minX, y)])
            } else {
                return Pathline([maxXMaxYPoint, other.minXMinYPoint])
            }
        }
    }
}
