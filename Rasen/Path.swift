...
struct Pathline {
    enum Element {
        ...
        var lastPoint: Point {
            switch self {
            case .linear(let p): p
            case .bezier(let p, _): p
            case .line(let line): line.lastPoint
            case .arc(let arc): arc.endPosition
            }
        }
    }
...
extension Pathline: AppliableTransform {
    static func * (lhs: Pathline, rhs: Transform) -> Pathline {
        let elements: [Element] = lhs.elements.map {
            switch $0 {
            case .linear(let p): .linear(p * rhs)
            case .bezier(let p, let cp): .bezier(point: p * rhs,
                                                 control: cp * rhs)
            case .line(let line): .line(line * rhs)
            case .arc(let arc): .arc(arc * rhs)
            }
        }
...
extension Pathline {
    ...
    var firstAngle: Double? {
        ...
        return switch fe {
        case .linear(let p): firstPoint.angle(p)
        case .bezier(let p, let cp):
            firstPoint.angle(firstPoint == cp ? p : cp)
        case .line(let line):
            firstPoint == line.firstPoint ?
            line.firstAngle : firstPoint.angle(line.firstPoint)
        case .arc(let arc):
            firstPoint == arc.startPosition ?
            arc.startAngle + (arc.orientation == .clockwise ? -.pi / 2 : .pi / 2) :
            firstPoint.angle(arc.startPosition)
        }
    }
    var lastAngle: Double? {
        ...
        case .arc(let arc):
            return arc.endAngle + (arc.orientation == .clockwise ? -.pi / 2 : .pi / 2)
        }
    }
    var lastPoint: Point {
        ...
        return switch lastElement {
        case .linear(let p1): p1
        case .bezier(let p1, _): p1
        case .line(let line): line.lastPoint
        case .arc(let arc): arc.endPosition
        }
    }
...
struct Path {
    ...
    init(_ edges: [Edge]) {
        pathlines = edges.map { .init($0) }
        updateBounds()
    }
    init(_ points: [Point]) {
        pathlines = [Pathline(points)]
        updateBounds()
    }
    init(_ polygon: Polygon) {
        if !polygon.isEmpty {
    ...
    init(_ topoly: Topolygon) {
        if !topoly.polygon.isEmpty {
            pathlines = topoly.holePolygons.isEmpty ?
                [Pathline(topoly.polygon)] :
                [Pathline(topoly.polygon)]
                    + topoly.holePolygons.compactMap { $0.isEmpty ? nil : Pathline($0) }
            isCap = false
            isTopolygon = true
            updateBounds()
        }
    }
    init(_ points: [Point], isClosed: Bool) {
        self.init([Pathline(points, isClosed: isClosed)])
    }
    init(_ pathlines: [Pathline], isPolygon: Bool = true, isCap: Bool = true, isTopolygon: Bool = false) {
...
extension Path {
    ...
    static func == (lhs: Path, rhs: Rect) -> Bool {
        guard lhs.pathlines.count == 1,
              lhs.pathlines[0].elements.count == 3 else { return false }
        let p0 = lhs.pathlines[0].firstPoint
        guard case .linear(let p1)
                = lhs.pathlines[0].elements[0] else { return false }
        guard case .linear(let p2)
                = lhs.pathlines[0].elements[1] else { return false }
        guard case .linear(let p3)
                = lhs.pathlines[0].elements[2] else { return false }
        return Polygon(points: [p0, p1, p2, p3]) == rhs
    }
    func inset(by w: Double) -> Path? {
        if let bounds = self.bounds, self == bounds {
            return Path(bounds.inset(by: w))
        } else {
            return nil
        }
    }
    ...
    func fillPointsData(withQuality quality: Double = 1) -> (pointsData: [Float],
        ...
        if isTopolygon {
            ...
            if pathlines.isEmpty {
                ...
                let polygons = pathlines.map { $0.polygon(withQuality: quality) }
                var floatPoints = [Float](), counts = [Int](), oldIndex = 0
                for polygon in polygons {
                    polygon.floatTriangles(in: &floatPoints,
                                           counts: &counts,
                                           oldIndex: &oldIndex)
                }
                return (floatPoints, counts)
...
extension Path {
    func linePointsDataWith(quality: Double = 1,
        ...
        func appendLinear(p0: Point, p1: Point,
            ...
            guard p0 != p1 else { return }
            let dp1 = (p1 - p0).perpendicularDeltaPoint(withDistance: s)
            if let p00 = p00,
               !p0.isApproximatelyEqual(p1, tolerance: 0.0001) {
                let e0 = Edge(p00, p0), e1 = Edge(p0, p1)
                let a = e0.angle() + .pi / 2
                let da = e0.angle(e1)
                appendCenterArc(Arc(centerPosition: p0,
                                    radius: s,
                                    startAngle: a, endAngle: a + da))
            }
            append(p0 - dp1)
            append(p0 + dp1)
            append(p1 - dp1)
            append(p1 + dp1)
            if let lastAngle = lastAngle,
               !p0.isApproximatelyEqual(p1, tolerance: 0.0001) {
                let e0 = Edge(p0, p1)
                let e1 = Edge(p1, p1.movedWith(distance: 1, angle: lastAngle))
                let a = e0.angle() + .pi / 2
                let da = e0.angle(e1)
                appendCenterArc(Arc(centerPosition: p1,
                                    radius: s,
                                    startAngle: a, endAngle: a + da))
            }
        }
    ...
    func lineColorsDataWith(_ colors: [Color], lineWidth lw: Double,
        ...
        func appendLinear(p0: Point, p1: Point, p00: Point? = nil,
                          lastAngle: Double? = nil,
                          k2: Int? = nil) {
            guard p0 != p1 else { return }
            if let p00 = p00,
        ...
        for pathline in pathlines {
            ...
            if pathline.isClosed {
                appendLinear(p0: p0, p1: pathline.firstPoint, p00: p00,
                             lastAngle: pathline.firstAngle, k2: fk)
